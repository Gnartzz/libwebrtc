...
void NvEncoder::CreateDefaultEncoderParams(NV_ENC_INITIALIZE_PARAMS* pIntializeParams, GUID codecGuid, GUID presetGuid)
{
    ...
    if (true)  // lossless presets removed in newer SDKs; never our path
    {
    ...
    {
        // removed: pixelBitDepthMinus8 (field gone in nv-codec-headers'
        // nvEncodeAPI.h; we never use HEVC 10-bit anyway).
...
void NvEncoder::CreateEncoder(const NV_ENC_INITIALIZE_PARAMS* pEncoderParams)
{
    ...
    if (pEncoderParams->encodeGUID == NV_ENC_CODEC_HEVC_GUID)
    {
        ...
        if (yuv10BitFormat && false /* pixelBitDepthMinus8 check removed */)
        {
    ...
    {
        NV_ENC_PRESET_CONFIG presetConfig = { NV_ENC_PRESET_CONFIG_VER, { NV_ENC_CONFIG_VER } };
        m_nvenc.nvEncGetEncodePresetConfig(m_hEncoder, pEncoderParams->encodeGUID, NV_ENC_PRESET_P3_GUID, &presetConfig);
