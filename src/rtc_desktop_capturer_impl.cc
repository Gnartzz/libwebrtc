...
RTCDesktopCapturerImpl::CaptureState RTCDesktopCapturerImpl::Start(
  ...
  if (type_ == kScreen) {
    ...
    if (!gpu_mode_ && !capturer_) {
      ...
      // Duplication konkurriert. AMD (#77): WGC-Bildschirm-Capturer. GDI lieferte
      // zwar ein echtes Bild, aber nur ~18 fps (CPU-BitBlt von 5120x1440). WGC
      // nutzt den GPU-Compositor (Grab ~12 ms -> bis 60 fps) und captured auch
      // Spiele/HW-Inhalte; vendor-neutral. Faellt WGC zur Laufzeit aus, waehlt
      // CreateScreenCapturer weiter DXGI/GDI.
      thread_->BlockingCall([this] {
#if defined(RTC_ENABLE_WIN_WGC)
        options_.set_allow_wgc_screen_capturer(true);
        HcCapLog("AMD-Fallback: WGC-Screen aktiviert");
#endif
        if (show_cursor_) {
          capturer_ = std::make_unique<webrtc::DesktopAndCursorComposer>(
              webrtc::DesktopCapturer::CreateScreenCapturer(options_), options_);
...
void RTCDesktopCapturerImpl::OnCaptureResult(
...
#endif
  {
    ...
    // AMD (#77): Vollbild-Screen ZUERST in ARGB herunterskalieren, erst das
    // kleine Bild nach I420 umwandeln. Spart auf grossen Panels (32:9 = 7,4
    // MPixel) die volle ConvertToI420 der Monitor-Aufloesung -> mehr fps. Nur
    // ohne Crop (x_/y_/w_/h_ = 0); sonst der generische Pfad darunter.
    webrtc::scoped_refptr<webrtc::I420BufferInterface> out_buffer;
    bool hc_fast = false;
#ifdef WEBRTC_WIN
    if (type_ == kScreen && x_ == 0 && y_ == 0 && w_ == 0 && h_ == 0 &&
        max_width_ > 0 && max_height_ > 0) {
      int src_w = rect_.width();
      int src_h = rect_.height();
      double s = static_cast<double>(max_width_) / src_w;
      double sy = static_cast<double>(max_height_) / src_h;
      if (sy < s) s = sy;
      if (s < 1.0 && src_w > 0 && src_h > 0) {
        int dst_w = (static_cast<int>(src_w * s)) & ~1;
        int dst_h = (static_cast<int>(src_h * s)) & ~1;
        if (dst_w > 1 && dst_h > 1) {
          size_t need = static_cast<size_t>(dst_w) * 4 * dst_h;
          if (hc_argb_scaled_.size() != need) hc_argb_scaled_.resize(need);
          libyuv::ARGBScale(frame->data(), frame->stride(), src_w, src_h,
                            hc_argb_scaled_.data(), dst_w * 4, dst_w, dst_h,
